...
class AslDb(object):
    """ American Sign Language database drawn from the RWTH-BOSTON-104 frame
    positional data
    ...
    feature_method definition named features, the following snippet may
    be used:
        asl = AslDb()
    ...
    The sentences provided in the data have been segmented into isolated words
    for this database
    """
    ...
    def __init__(self,
        ...
        """ loads ASL database from csv files with hand position information
        by frame, and speaker information
        ...
        """
        self.df = pd.read_csv(hands_fn).merge(pd.read_csv(speakers_fn),
                                              on='video')
        self.df.set_index(['video', 'frame'], inplace=True)

    def build_training(self, feature_list, csvfilename=None):
        """ wrapper creates sequence data objects for training words suitable
        for hmmlearn library
        ...
        """
        if not csvfilename:
            csvfilename = os.path.join(PATH, 'train_words.csv')
        return WordsData(self, csvfilename, feature_list)

    def build_test(self, feature_method, csvfile=None):
        """ wrapper creates sequence data objects for individual test word
        items suitable for hmmlearn library
        ...
        """
        if not csvfile:
            csvfile = os.path.join(PATH, 'test_words.csv')
        return SinglesData(self, csvfile, feature_method)


class WordsData(object):
    """ class provides loading and getters for ASL data suitable for use with
    hmmlearn library
    """

    def __init__(self, asl: AslDb, csvfile: str, feature_list: list):
        """ loads training data sequences suitable for use with hmmlearn
        library based on feature_method chosen

        :param asl: ASLdata object.
        :param csvfile: str. filename of csv file containing word training
            start and end frame data with expected format: video,speaker,word,
            startframe,endframe
    ...
    def _load_data(self, asl, fn, feature_list):
        ...
        for i in range(len(tr_df)):
            ...
            for frame in range(tr_df.ix[i, 'startframe'],
    ...
    def get_all_sequences(self):
        """ getter for entire db of words as series of sequences of feature
    ...
    def get_all_Xlengths(self):
        """ getter for entire db of words as (X, lengths) tuple for use with
        hmmlearn library

        :return: dict. dictionary of (X, lengths) tuple, where X is a numpy
            array of feature lists and lengths is a list of lengths of
            sequences within X
                {'FRANK': (array([[ 87, 225],[ 87, 225], ...  [ 87, 225,  62,
                                  127], [ 87, 225,  65, 128]]), [14, 18]),
                ...}
        """
        return self._hmm_data

    def get_word_sequences(self, word: str):
        """ getter for single word series of sequences of feature lists for
        each frame
    ...
    def get_word_Xlengths(self, word: str):
        """ getter for single word (X, lengths) tuple for use with hmmlearn
        library

        :param word:
        :return: (list, list). (X, lengths) tuple, where X is a numpy array of
        feature lists and lengths is a list of lengths of sequences within X
                (array([[ 87, 225],[ 87, 225], ...  [ 87, 225,  62, 127], [ 87,
                        225,  65, 128]]), [14, 18])
        """
...
class SinglesData(object):
    ...
    def _load_data(self, asl, feature_list):
        """ Consolidates sequenced feature data into a dictionary of words and
        creates answer list of words in order
        ...
        for i in range(len(self.df)):
            ...
            for frame in range(self.df.ix[i, 'startframe'],
                               self.df.ix[i, 'endframe']+1):
                vid_frame = video, frame
                sample = [asl.df.ix[vid_frame][f] for f in feature_list]
                if len(sample) > 0:  # dont add if not found
                    new_sequence.append(sample)
    ...
    def _load_sentence_word_indices(self):
        """ create dict of video sentence numbers with list of word indices as
        values

        :return: dict. {v0: [i0, i1, i2], v1: [i0, i1, i2], ... ,} where v#
                        is video number and i# is index to wordlist, ordered by
                        sentence structure
        """
    ...
    def get_all_sequences(self):
        """ getter for entire db of items as series of sequences of feature
        lists for each frame

        :return: dict dictionary of lists of feature list sequence lists for
            each indexed item
    ...
    def get_all_Xlengths(self):
        """ getter for entire db of items as (X, lengths) tuple for use with
        hmmlearn library

        :return: dict. dictionary of (X, lengths) tuple, where X is a numpy
            array of feature lists and lengths is a list of lengths of
            sequences within X; should always have only one item in lengths
                {3: (array([[ 87, 225],[ 87, 225], ...  [ 87, 225,  62, 127],
                [ 87, 225,  65, 128]]), [14]),
                ...}
        """
        return self._hmm_data
    ...
    def get_item_Xlengths(self, item: int):
        ...
        :param word:
        :return: (list, list) (X, lengths) tuple, where X is a numpy array of
            feature lists and lengths is a list of lengths of sequences within
            X; lengths should always contain one item
                (array([[ 87, 225],[ 87, 225], ...  [ 87, 225,  62, 127],
                [ 87, 225,  65, 128]]), [14])
        """
